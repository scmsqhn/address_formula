...
# ====================== model inout=====================
    ...
    def init_embedding(self):
        ...
        with tf.device("/cpu:0"):
            self.embedding = tf.get_variable("embedding", [self.timestep_size, self.embedding_size], dtype=tf.int32)
    ...
    def init_bi_lstm(self, vocab_size, embedding_size, X_inputs):
            ...
            #self.datahelper.fromvec(X_inputs)
            _inputs = self.matmul(tf.cast(tf.reshape(X_inputs,(6400,128)),tf.float32), tf.cast(embedding,tf.float32), embedding_size, dtype=tf.float32)
            inputs = tf.reshape(_inputs, (32,200,128))
    ...
    def init_model_struct(self):
        ...
        self.embedding = tf.Variable(tf.random_uniform([self.embedding_size, self.embedding_size], -10.0, 10.0))
    ...
    def init_attention(self, X_inputs, y_pred, vocab_size, btsize, timestep_size, embedding_size, class_num):
            ...
            #embedding = tf.Variable(tf.random_uniform([vocab_size, embedding_size], -1.0, 1.0))
            embedding = self.embedding
            att_inputs = self.matmul(tf.cast(tf.reshape(X_inputs,(6400,128)),tf.float32), tf.cast(self.embedding,tf.float32), embedding_size, dtype=tf.float32)
