...
class Data_Helper(object):
    ...
    def fromvec(self,wordis): # [,128] np.array
        wordvec = ""
        docWord = wordis#self.fromdct(word)
        try:
            wordvec = self.w2vm[docWord]
        except:
            wordvec = self.w2vm[" "]
            #wordvec = np.array([-1.0]*128)
        return list(wordvec)
    ...
    def next_batch_eval(self,gen):
        ...
        while(1):
            ...
            print("\n>counter:",i)
            if i==self.btsize:
                yield self.toArr(_ids,self.btsize,ssl), self.toArr(_tags,self.btsize,200), self.toArr(_words, self.btsize,200)
                if Const.DEBUG=="True":
        ...
        while(1):
            ...
            try:
                ...
                if round_cnt%self.btsize==0:
                    ...
                    # pdb.set_trace()
                    _x = np.array(_ids)
                    _x = _x.reshape(self.btsize, 200*128)
                    _y = np.array(_tags)
                    _y = _y.reshape(self.btsize, 200)
                    yield  _x, _y
                    _ids,_tags = [],[]
